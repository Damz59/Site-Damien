...
import "./Admin_CVCompetences_style.css"
...
function AdminCVPublicPreview({ refreshKey, sections }) {
	...
	return (
		<Card className="admin-cv-preview-card shadow-sm">
			<Card.Body>
				<div className="admin-cv-preview-header">
					<h2 className="h4 mb-0">
						Aperçu public (comme la page CV / Compétences)
					</h2>
					<Badge bg="secondary">Live</Badge>
				</div>

				<hr className="admin-cv-preview-divider" />
				...
				{!loading && !error && (
					<div className="d-flex flex-column gap-3">
						{visibleSections.length === 0 ? (
							<Card className="admin-cv-section-card shadow-sm">
								<Card.Body>
							...
							visibleSections.map((s) => {
								const sectionData = data[s.section] || {
									sectionActive: 1,
									items: [],
								}
								...
								return (
									<Card key={s.section} className="admin-cv-section-card shadow-sm">
										<Card.Body>
											<div className="d-flex align-items-center justify-content-between gap-2">
												<Card.Title className="admin-cv-section-title mb-0">
													{s.label || s.section}
												</Card.Title>
											...
											{items.length === 0 ? (
												...
												<ul className="admin-cv-list mb-0">
													{items.map((it) => (
														<li key={it.id}>
															{it.text}
															{it.description ? (
	...
	return (
		<main className="page-content flex-grow-1 overflow-auto">
			...
			<Container className="my-5">
				<h1 className="admin-cv-title mb-4">Administration — CV / Compétences</h1>
				...
				<div className="admin-cv-actions-bar d-flex justify-content-end gap-2 mb-2">
